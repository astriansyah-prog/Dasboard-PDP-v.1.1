...
def clean_multiline(v):
    """Untuk teks yang berisi beberapa baris (ST-1/ST-2/RLB/SLO/status PRK/
    keterangan dsb. di sheet List Project_ExAPBN): rapikan spasi PER BARIS
    (termasuk spasi ekstra sisa hasil export Google Sheets), tapi baris baru
    tetap dipertahankan supaya butir-butir informasinya tetap terpisah rapi.
    Tanggal -> 'dd Mon yyyy'. Baris kosong dibuang."""
    if v is None:
        return None
    if isinstance(v, datetime.datetime):
        return v.strftime("%d %b %Y")
...
def numf_flex(v):
    """Parse angka Rp yang bisa berupa float/int asli (dari file .xlsx hasil
    download manual) ATAU teks berformat Indonesia (dari live export Google
    Sheets API, mis. '3.663.276.197') — dipakai khusus sheet List Project_ExAPBN
    karena kedua bentuk ini terbukti muncul tergantung sumber datanya."""
    if v is None:
        return 0.0
    if isinstance(v, (int, float)):
        return float(v)
...
def bucket_yn(v):
    """Normalisasi jawaban Ada/Tidak Ada yang ejaannya tidak konsisten di
    sheet List Project_ExAPBN (Ada/ADA, Tidak/Tidak ada/Tidak Ada/TIDAK, dst)
    menjadi 3 kategori baku untuk kebutuhan infografis."""
    if v is None:
        return "N/A"
    s = str(v).strip().lower()
...
def bucket_prk(v):
    """Khusus kolom 'Status dan Nomor PRK': banyak sel berisi langsung nomor
    PRK (tanpa awalan 'Ada') yang tetap berarti PRK-nya ADA."""
    if v is None:
        return "N/A"
    s = str(v).strip().lower()
    if not s or s in ("n/a", "-", "none"):
...
def bucket_perusahaan(v):
    """Normalisasi kolom 'Status Perusahaan' (Aktif/Non Aktif/Pailit) yang
    ejaannya bervariasi (AKTIF/Aktif, NON AKTIF, dst.) atau berisi beberapa
    kontraktor sekaligus (mis. 'PT A (Pailit), PT B (Aktif)')."""
    if v is None:
        return "Lainnya"
    s = str(v).strip().lower()
    if not s or s in ("none", "?", "???"):
        return "Lainnya"
    has_nonaktif = "non aktif" in s or "nonaktif" in s
    has_pailit = "pailit" in s
    has_aktif = ("aktif" in s) and not has_nonaktif
    if has_pailit and (has_aktif or has_nonaktif or "," in s):
        return "Campuran"
    if has_pailit:
        return "Pailit"
    if has_nonaktif:
...
def bucket_kondisi(v):
    """Normalisasi kolom 'Kondisi Actual' (bebas teks) ke 4 kategori ringkas
    untuk grafik infografis; teks asli tetap disimpan apa adanya per-item."""
...
def parse_ex_apbn(wb):
    """Sheet 'List Project_ExAPBN': daftar legacy proyek eks-APBN, dikelompokkan
    per UIP lewat baris judul 'UIP <NAMA>' (tanpa nomor urut) yang diselingi
    baris proyek bernomor urut ("No" numerik di kolom A).

    Header sheet ini menyebar di 2-3 baris sekaligus dengan banyak sel gabungan
    (mis. "SURAT KE BPKP" menaungi 2 sub-kolom PA/KLAIM, "STATUS PROYEK"
    menaungi 5 sub-kolom ST-1/ST-2/RLB/SLO/STOP-STAP-STP), dan posisi baris
    header ini bergeser 1 baris tergantung sumbernya (file .xlsx yang
    didownload manual vs. live export dari Google Sheets API). Karena itu kita
    cari baris header secara dinamis (baris yang sel kolom A-nya persis "NO"),
    lalu kolom-kolom berikutnya diambil berdasarkan URUTAN TETAP — sheet ini
    adalah formulir baku (template pelaporan BPKP) sehingga urutan kolom A-X
    selalu sama meski baris header bisa bergeser."""
    ws = wb["List Project_ExAPBN"]

    header_row_idx = None
    for r in range(1, 12):
        v = ws.cell(r, 1).value
        if v is not None and str(v).strip().upper() == "NO":
            header_row_idx = r
            break
    if header_row_idx is None:
        raise RuntimeError("Header 'NO' tidak ditemukan di sheet List Project_ExAPBN")

    # Kolom tetap (lihat docstring): A=NO, B=FUNGSI, C=NOMOR KONTRAK, ...
    C_FUNGSI, C_KONTRAK, C_TGLKONTRAK, C_KONTRAKTOR, C_STATUSP, C_PEKERJAAN = 2, 3, 4, 5, 6, 7
    C_BPKP_PA, C_BPKP_KLAIM = 8, 9
    C_MKS_PA, C_MKS_KLAIM, C_MKS_STATUS = 10, 11, 12
    C_KONDISI = 13
    C_ST1, C_ST2, C_RLB, C_SLO, C_STOPSTAPSTP = 15, 16, 17, 18, 19
    C_KLAUSUL_PA, C_KLAUSUL_FAC, C_STATUS_PRK, C_SISA_AI = 20, 21, 22, 23
    C_KETERANGAN = 24
    ...
    for r in range(header_row_idx + 1, ws.max_row + 1):
        ...
        if isinstance(a, str):
            ...
            su = s.upper()
            if su == "TOTAL":
                break  # akhir data proyek, sisanya catatan kaki
            if su.startswith("UIP"):
                raw_key = re.sub(r"^UIP\s*", "", s, flags=re.I).strip().upper()
                current_unit_key = raw_key if raw_key in BY_KEY else raw_key
                continue
            continue  # baris teks lain (catatan) -> lewati
        if not isinstance(a, (int, float)):
            continue
        nama_pekerjaan = ws.cell(r, C_PEKERJAAN).value
        if not nama_pekerjaan:
            continue
        items.append({
            "no": int(a), "unit": current_unit_key,
            "fungsi": text_strip(ws.cell(r, C_FUNGSI).value),
            "nomor_kontrak": text_strip(ws.cell(r, C_KONTRAK).value),
            "tanggal_kontrak": text_strip(ws.cell(r, C_TGLKONTRAK).value),
            "kontraktor": text_strip(ws.cell(r, C_KONTRAKTOR).value),
            "status_perusahaan": text_strip(ws.cell(r, C_STATUSP).value),
            "nama_pekerjaan": text_strip(nama_pekerjaan),
            "bpkp_pa": numf_flex(ws.cell(r, C_BPKP_PA).value),
            "bpkp_klaim": numf_flex(ws.cell(r, C_BPKP_KLAIM).value),
            "mks_pa": numf_flex(ws.cell(r, C_MKS_PA).value),
            "mks_klaim": numf_flex(ws.cell(r, C_MKS_KLAIM).value),
            "mks_status": text_strip(ws.cell(r, C_MKS_STATUS).value),
            "kondisi_actual": text_strip(ws.cell(r, C_KONDISI).value),
            "st1": clean_multiline(ws.cell(r, C_ST1).value),
            "st2": clean_multiline(ws.cell(r, C_ST2).value),
            "rlb": clean_multiline(ws.cell(r, C_RLB).value),
            "slo": clean_multiline(ws.cell(r, C_SLO).value),
            "status_stop_stap_stp": clean_multiline(ws.cell(r, C_STOPSTAPSTP).value),
            "klausul_pa_klaim": text_strip(ws.cell(r, C_KLAUSUL_PA).value),
            "klausul_fac": text_strip(ws.cell(r, C_KLAUSUL_FAC).value),
            "status_prk": clean_multiline(ws.cell(r, C_STATUS_PRK).value),
            "sisa_ai": numf_flex(ws.cell(r, C_SISA_AI).value),
            "keterangan": clean_multiline(ws.cell(r, C_KETERANGAN).value),
        })
    return items
...
def main():
    ...
    print("[build_data] Parsing List Project_ExAPBN ...")
    ex_apbn_items = parse_ex_apbn(wb)
    print(f"    -> {len(ex_apbn_items)} proyek")
    ...
    ea_paklaim_dist = {}
    ea_prk_dist = {}
    for it in ex_apbn_items:
        f = it["fungsi"] or "Lainnya"
        ea_by_fungsi[f] = ea_by_fungsi.get(f, 0) + 1
        ea_kondisi_dist[bucket_kondisi(it["kondisi_actual"])] = ea_kondisi_dist.get(bucket_kondisi(it["kondisi_actual"]), 0) + 1
        ea_statusp_dist[bucket_perusahaan(it["status_perusahaan"])] = ea_statusp_dist.get(bucket_perusahaan(it["status_perusahaan"]), 0) + 1
        ea_fac_dist[bucket_yn(it["klausul_fac"])] = ea_fac_dist.get(bucket_yn(it["klausul_fac"]), 0) + 1
        ea_paklaim_dist[bucket_yn(it["klausul_pa_klaim"])] = ea_paklaim_dist.get(bucket_yn(it["klausul_pa_klaim"]), 0) + 1
        ea_prk_dist[bucket_prk(it["status_prk"])] = ea_prk_dist.get(bucket_prk(it["status_prk"]), 0) + 1
    ea_units = sorted(set(it["unit"] for it in ex_apbn_items if it["unit"]))
    ex_apbn = {
        "count": len(ex_apbn_items), "unit_count": len(ea_units),
        "by_fungsi": ea_by_fungsi, "kondisi_dist": ea_kondisi_dist,
        "status_perusahaan_dist": ea_statusp_dist, "klausul_fac_dist": ea_fac_dist,
        "klausul_pa_klaim_dist": ea_paklaim_dist, "status_prk_dist": ea_prk_dist,
        "sisa_ai_total": sum(it["sisa_ai"] for it in ex_apbn_items),
        "items": ex_apbn_items,
    }
