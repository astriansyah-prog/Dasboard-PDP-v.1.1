...
berkendala, daftar legacy **Proyek Ex-APBN**, dan rekonsiliasi nilai PDP DIV
PMO vs DIV AKT (SAP). Tampilan & logikanya mengikuti dashboard asli yang
diupload, namun datanya sekarang diambil langsung dari Google Spreadsheet
sehingga selalu bisa diperbarui tanpa mengedit HTML.
...
1. `build_data.py` membaca spreadsheet sumber (sheet: `SAP-DIVAKT`,
   ...
   `Biaya Ditangguhkan`, `List Project_ExAPBN`) dan menghitungnya menjadi
   satu file `data/pdp-data.json` — persis skema data yang dipakai dashboard.
...
| `List Project_ExAPBN`      | Daftar legacy proyek era APBN (kontrak, kontraktor, progres ST-1/ST-2/RLB/SLO/STOP-STAP-STP, klausul FAC/PRK) |

Khusus sheet `List Project_ExAPBN`: formatnya adalah formulir baku (template
pelaporan BPKP) dengan header yang menyebar 2-3 baris dan proyek dikelompokkan
per UIP lewat baris judul `UIP <NAMA>` (tanpa nomor urut) yang diselingi baris
proyek bernomor. `build_data.py` mencari baris header secara otomatis (baris
yang kolom pertamanya persis "NO"), lalu kolom-kolom berikutnya diambil sesuai
urutan tetap A-X pada template ini. Kalau BPKP mengubah urutan kolom di
template, sesuaikan konstanta `C_FUNGSI`, `C_KONTRAK`, dst. di fungsi
`parse_ex_apbn()` pada `build_data.py`.
...
Menu ini (di bawah "Cluster Proyek 1-9") menampilkan legacy proyek era APBN
yang datanya diambil dari sheet `List Project_ExAPBN`: kartu infografis
(total proyek, sudah beroperasi, jenis proyek terbanyak, jumlah proyek yang
belum ada amandemen FAC, sisa anggaran investasi), grafik distribusi per
fungsi/kondisi/unit/status perusahaan kontraktor, serta tabel proyek yang
bisa dicari & difilter per UIP/fungsi. Klik satu baris untuk melihat detail
lengkap: nomor & tanggal kontrak, kontraktor, status proyek, progres serah
terima (ST-1/ST-2/RLB/SLO/STOP-STAP-STP), klausul PA/Klaim/FAC, status PRK,
dan keterangan tambahan.

Karena menu ini baru (tidak ada di dashboard versi awal yang diupload),
beberapa nilai teks yang ejaannya tidak konsisten di spreadsheet asal (mis.
"Aktif"/"AKTIF", "Tidak ada"/"Tidak Ada"/"TIDAK") dirapikan/dikelompokkan
otomatis oleh `build_data.py` khusus untuk kebutuhan grafik ringkasan —
teks asli apa adanya tetap ditampilkan di tabel & detail per proyek.
...
dikenal, baris "NIHIL"/"TOTAL", dsb). Data `List Project_ExAPBN` (menu
"Proyek Ex-APBN") divalidasi terpisah karena menu ini baru: jumlah baris,
pengelompokan per UIP, dan seluruh field diperiksa manual terhadap sheet
sumber (104 proyek di 8 UIP).
